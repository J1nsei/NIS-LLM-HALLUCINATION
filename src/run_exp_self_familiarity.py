...
if __name__ == "__main__":
    ...
    data_file = 'data/data_knowledge_{}_general_multi_small.csv'.format(args.exp_file)
...
""".format(
    ...
    for para in para_list:
        ...
        print('\nMPT\n', mpt)
        model = AutoModelForCausalLM.from_pretrained("{mpt}".format(mpt=args.model), cache_dir='/media/j1nsei/SSD/LLMs/data/cache',
                                                     trust_remote_code=True, device_map="auto",
                                                     max_memory=max_memory, torch_dtype=torch.float16)
        model.eval()
        if 'alpaca' in args.model:
            tokenizer = AutoTokenizer.from_pretrained("{mpt}".format(mpt=args.model), cache_dir='/media/j1nsei/SSD/LLMs/data/cache',device_map="auto",
                                                      unk_token="<unk>",
                                                      bos_token="<s>",
                                                      eos_token="</s>")
        else:
            tokenizer = AutoTokenizer.from_pretrained("{mpt}".format(mpt=args.model), device_map="auto",cache_dir='/media/j1nsei/SSD/LLMs/data/cache')
